...
今回はクラスの継承を行うことで処理をより簡潔にする。

### 1. 現状確認
現状のクラス構成を確認すると、`Wine`と`Beer`の中身は全く同じである。
そのため、クラスの継承を用いることによって処理を簡潔にしていきたい。

### 2. クラスの書き換え
現状`Wine`クラスと`Beer`クラスはrecordで定義されている。しかしこの状態では継承が出来ない。
そのため、`Wine`クラスと`Beer`クラスを`class`で定義してください。

その際、クラスではrecordで省略されているコンストラクタやフィールド変数の定義が必要なことに注意してください。


### 3. Drinkクラスの定義
`Wine`クラスと`Beer`クラスは構成が殆ど同じであるため、`Drink`クラスを定義し、それを継承する形にしたい。  
その際、共通する処理は全て親クラス(`Drink`)に定義し、子クラス(`Wine`, `Beer`)にはそれぞれの特有の処理のみを定義すること。  
(今回は共通の部分しか無いため、子クラスの中身はコンストラクタだけです。)
...
### 4. 動作確認
前回までの適当なエンドポイントをpostmanで呼び出し、動作確認を行ってください。
